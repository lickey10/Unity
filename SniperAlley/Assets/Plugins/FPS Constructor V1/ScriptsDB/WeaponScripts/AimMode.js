...
public var keepAiming : boolean = false;
...
function Update(){	
	...
	if (InputDB.GetButtonDown("Aim"))
	    keepAiming = !keepAiming;

    //aiming
	if (InputDB.GetButton("Aim") && canAim && PlayerWeapons.canAim && selected && !sprinting /*&& !GunScript1.sprint*/ && Avoidance.canAim || keepAiming){	
...
function GunToRotation(v3 : Vector3, rate : float){
    ...
    function KeepAiming(isAiming:boolean)
    {
        keepAiming = isAiming;
    }