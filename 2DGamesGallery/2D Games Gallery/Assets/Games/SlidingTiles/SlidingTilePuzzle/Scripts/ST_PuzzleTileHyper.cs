...
public class ST_PuzzleTileHyper : MonoBehaviour 
{
	...
	{
        StartCoroutine(UpdatePosition(10f));
    ...
    public void ExecuteAdditionalMove()
	{
		...
		LaunchPositionCoroutine(this.transform.parent.GetComponent<ST_PuzzleDisplayHyper>().GetTargetLocation(this.GetComponent<ST_PuzzleTileHyper>()));
	}

    public void ExecuteAdditionalMove(float speed)
    {
        // get the puzzle display and return the new target location from this tile. 
        LaunchPositionCoroutine(this.transform.parent.GetComponent<ST_PuzzleDisplayHyper>().GetTargetLocation(this.GetComponent<ST_PuzzleTileHyper>()), speed);
    }
