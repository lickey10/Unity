...
public class ST_PuzzleDisplayHyper : MonoBehaviour 
{
	// this puzzle texture.
	...
	private ST_PuzzleTileHyper CheckMoveLeft(int Xpos, int Ypos, ST_PuzzleTileHyper thisTile)
	{
		// move left 
		if((Xpos - 1)  >= 0)
		{
	...
	private IEnumerator JugglePuzzle()
	{
		...
		TileDisplayArray[0,0].GetComponent<ST_PuzzleTileHyper>().Active = false;
        ...
        for (int k = 0; k < 20; k++)
        {
            ...
            for (int j = 0; j < Height; j++)
            {
                for (int i = 0; i < Width; i++)
                {
                    ...
                    TileDisplayArray[i, j].GetComponent<ST_PuzzleTileHyper>().ExecuteAdditionalMove();
	...
	public IEnumerator CheckForComplete()
	{
		while(Complete == false)
		{
			...
			for(int j = Height - 1; j >= 0; j--)
			{
				...
				{
					// check if this tile has the correct grid display location.
					if(TileDisplayArray[i,j].GetComponent<ST_PuzzleTileHyper>().CorrectLocation == false)  
					{
						Complete = false;
					}
				}
