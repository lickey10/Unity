...
public class LevelMenu : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		GUI.Box(new Rect( 3, 3, 375, 30 ), gamestate.Instance.GameName ,customGuiStyle);
		
		customGuiStyle.normal.textColor = Color.black;
		GUI.Box(new Rect( 0, 0, 375, 30 ), gamestate.Instance.GameName ,customGuiStyle);

		//levels
		//drop shadow
		customGuiStyle.normal.textColor = Color.red;
		GUI.Box(new Rect( 3, 43, 375, 30 ), "Levels" ,customGuiStyle);

		customGuiStyle.normal.textColor = Color.black;
		GUI.Box(new Rect( 0, 40, 375, 30 ), "Levels" ,customGuiStyle);
			...
			for (int y = 0; (x*MaxColumns) + y < gamestate.Instance.GetNumberOfLevels(); y++)
			{
				...
				else //unlocked
				{
					...
					if (GUI.Button(new Rect(y*buttonWidth+buttonPadding,x*buttonHeight+buttonPadding,buttonWidth-5,buttonHeight-5),((x*MaxColumns)+(y+1)).ToString(),customGuiStyle))
					{
						...
						Application.LoadLevel("level"+ selectedLevel.ToString()+"StartMenu");
