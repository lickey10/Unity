	...
	void OnGUI()
	{
		...
		if (DisplayText)
		{
			...
			if(TextString.Length == 0)
				tempTextString = ((int)(Value * 100)).ToString() + "%";
			else
				tempTextString = TextString;

			//capital Y of webdings is a heart

			GUI.Label(new Rect(Position.x + TextOffset.x,Position.y + TextOffset.y,ValueBar.width * ScaleSize,ValueBar.height * ScaleSize),tempTextString,LabelStyle);
