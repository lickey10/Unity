...
private var ButtonToPress : KeyCode = KeyCode.E; //The button to press when picking up the item.
...
var DisplayPickupMessage : boolean = true;
...
function Awake ()
{
	...
	ButtonToPress = theItem.PickupButton;
	...
	if(DisplayPickupMessage && PickupMessage.length == 0)
		PickupMessage = "Press "+ ButtonToPress.ToString() +" to pick up " + transform.name + ".";
}
...
function OnGUI ()
{
	...
	if (DisplayPickupMessage && canPickUp == true)
	{
		if (transform.name.Length <= 7)
		{
			GUI.Box (Rect (Screen.width*0.5-(165*0.5), 200, 165, 22), PickupMessage);
		}
		else
		{
			GUI.Box (Rect (Screen.width*0.5-(185*0.5), 200, 185, 22), PickupMessage);
		}
	}
}
