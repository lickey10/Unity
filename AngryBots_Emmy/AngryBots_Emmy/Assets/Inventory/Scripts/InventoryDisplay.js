...
private var cSheet : CharacterInventory;
...
function Awake()
{
	...
	if (GetComponent(CharacterInventory) != null)
	{
		cSheetFound = true;
		cSheet = GetComponent(CharacterInventory);
	}
	else
	{
		...
		if (cSheetFound) //CSheet was found (recommended)
		{
			if(GUI.Button(Rect(currentX,currentY,itemIconSize.x,itemIconSize.y),item.itemIcon))
			{
				...
				if(itemBeingDragged == item) //We clicked the item, then clicked it again
				{
					if (cSheetFound)
					{
						GetComponent(CharacterInventory).UseItem(item,0,true); //We use the item.
					}
