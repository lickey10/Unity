...
public class doorscript : MonoBehaviour {
	...
	//check distance from player
		...
		if(playerdist<distancetoopen){
			...
			if(dooropen){
						...
						if(aitoenable){
							...
							GetComponent<Animation>()[dooropen.name].time = 0;
		GetComponent<Animation>()[dooropen.name].speed = GetComponent<Animation>()[dooropen.name].length / 0.8f;
			GetComponent<Animation>().Play( dooropen.name);	
			dooractive=false;
		...
		GetComponent<Animation>()[dooropen.name].time = GetComponent<Animation>()[dooropen.name].length;
		GetComponent<Animation>()[dooropen.name].speed = GetComponent<Animation>()[dooropen.name].length / -0.8f;
			GetComponent<Animation>().Play( dooropen.name);	
				dooractive=false;
							}	
