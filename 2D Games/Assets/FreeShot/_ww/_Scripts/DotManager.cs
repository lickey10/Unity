...
public class DotManager : MonoBehaviour 
{
	...
	void Reset()
	{
		...
		DotSprite.color = GameManager3.Instance.currentAAColor.colorDark;
	...
	void OnCollisionEnter2D(Collision2D col)
	{
		...
		{
            GameManager3.Instance.OnDespawnedDotOnTarget (transform);
            GameManager3.Instance.lastShoots.Remove(this);
		}
		else
		{
	...
	{
		if( !GameManager3.Instance.isGameOver && !isOnCircle)
		{
			GetComponent<Rigidbody2D>().velocity = Vector3.zero;
