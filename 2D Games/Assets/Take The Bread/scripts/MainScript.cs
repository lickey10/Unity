...
public class MainScript : MonoBehaviour {
		...
		void initView(){
				...
				GameManager2.getInstance ().CacheInterestial ();
		...
		public void gameWin(bool directWin = false){
				...
				GameManager2.getInstance ().playSfx ("applaud");
				StartCoroutine ("win");
		...
		IEnumerator win(){
				...
				if (tclv % 3 == 1 && tclv > 6) {
						GameManager2.getInstance ().ShowInterestitial ();
				}
				...
				if (cLvScore < tScore) {
						...
						if(GameData.getInstance().lvStar.Count != 0){
								...
								GameManager2.getInstance().submitGameCenter();
		...
		public void gameFailed(){
				...
				GameData.getInstance ().lockGame (true,false);
				GameManager2.getInstance ().playSfx ("gamelose");
		...
		public void buttonHandler(GameObject g){
				...
				case "btnMain":
						...
						GameManager2.getInstance ().playSfx ("click");
						break;
				case "btnLevel":
