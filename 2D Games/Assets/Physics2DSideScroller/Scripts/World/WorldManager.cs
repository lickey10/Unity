...
public class WorldManager : MonoBehaviour
{
    ...
    private void GamePause()
    {
        ...
        {
            Time.timeScale = 0;
            CheeseHead.GetComponent<PlayerController2>().enabled = CheeseHead.GetComponentInChildren<AttackController>().enabled = false;
        }
        else if (!PauseSwitch && !GameOverManager.IsGameOver)
        {
            Time.timeScale = 1;
            CheeseHead.GetComponent<PlayerController2>().enabled = CheeseHead.GetComponentInChildren<AttackController>().enabled = true;
        }
    }
}