...
////////// Sway //////////
var sway : boolean = false; //Does the weapon sway?
...
function LateUpdate(){
	...
	if(gunActive){
		if(idleTime > timeToIdle){
			if(CM !=null && !aim1.aiming && !Avoidance.collided){
	...
	if(CM != null && CM.walking && !aim1.aiming && sway && !CharacterMotorDB.paused){
...
function FireOneBullet(){  
    ...
    var dist : float = 1000;
  	...
  	if(tracer != null && traceEvery <= shotCountTracer && traceEvery != 0){
  		var emitter : ParticleSystem = tracer.GetComponent(ParticleSystem);
  		shotCountTracer = 0;
  		if(hits.length > 0){
