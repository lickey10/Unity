...
[CustomEditor (typeof(JSFBoardLayout))]
public class JSFBoardLayoutEditor : Editor {
	...
	public JSFPanelDefinition[] scripts;
	public JSFPieceDefinition[] pieces;
	...
	void drawLayoutTable()
	{
		...
		if(panelArray != null && !Application.isPlaying){
			...
			for(int x = 0; x < Mathf.Min( bl.gm.boardWidth, 20); x++){
				...
				if(x%2 == 1 && bl.showHexGrid && bl.gm.boardType == JSFBoardType.Hexagon){
				...
				for(int y = 0; y < Mathf.Min( bl.gm.boardHeight, 20) ; y++){
					...
					// weights selection
					if(bl.colorArray[count] == 0){ // show weights option if no color grouping...
						if(bl.colorWeight[count].useWeights){
							GUI.backgroundColor = Color.green;
						}
						if(GUILayout.Button( "w", new GUILayoutOption[] {GUILayout.Width(22),GUILayout.Height(18)} ) ){
							EditorWindow.GetWindow (typeof (JSFBoardLayoutWeightsWindow),false, "Set Weights");
							JSFBoardLayoutWeightsWindow.setDisplayType(count);
						}
					}
					GUI.backgroundColor = Color.white; // back to normal color for other GUIs
					EditorGUILayout.EndHorizontal();
					...
					if(bl.panelEditVisuals[num] != null && bl.panelEditVisuals[num].texture != null){ // assigned texture version
						if(GUILayout.Button( bl.panelEditVisuals[num].texture,GUI.skin.box, layoutParams ) ){
							if(selectorSwitch){
								EditorWindow.GetWindow (typeof (JSFBoardLayoutChooserWindow),false, "Choose Type");
								JSFBoardLayoutChooserWindow.setDisplayType(count,false);
							} else{
								if (Event.current.button == 0)
									bl.togglePanel(count,1);
					...
					if(bl.pieceEditVisuals[type] != null && bl.pieceEditVisuals[type].texture != null){ // assigned texture version
						if(GUILayout.Button( bl.pieceEditVisuals[type].texture, GUI.skin.label, layoutParams ) ){
							if(selectorSwitch){
								EditorWindow.GetWindow (typeof (JSFBoardLayoutChooserWindow),false, "Choose Type");
								JSFBoardLayoutChooserWindow.setDisplayType(count,true);
							} else{
								if (Event.current.button == 0)
									bl.togglePiece(count,1);
