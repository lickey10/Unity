...
//stuff=gameObject.GetComponent("MouseTimeLook");
		...
		GetComponent<Animation>()[death.name].speed = GetComponent<Animation>()[death.name].length / 2f;
		...
		if(run){
				if(charge){
		GetComponent<Animation>()[run.name].speed = GetComponent<Animation>()[run.name].length / 0.8f;
			GetComponent<Animation>().CrossFade( run.name, 0.12f);	
	...
	//idle animation
			if(idleattack){
				if(idle){
		GetComponent<Animation>()[idleattack.name].speed = GetComponent<Animation>()[idleattack.name].length / 2f;
			GetComponent<Animation>().CrossFade( idleattack.name, 0.25f);
		...
		if(attackanim){
				if(attack){
		GetComponent<Animation>()[attackanim.name].speed = GetComponent<Animation>()[attackanim.name].length / 1f;
			GetComponent<Animation>().CrossFade( attackanim.name, 0.15f);
		...
		Debug.DrawLine(playertarget.transform.position, attackrange3.transform.position, Color.red);
			...
			if(enableattackdam){
					...
					if(Physics.Linecast(playertarget.transform.position, attackrange.transform.position, out hit)|Physics.Linecast(playertarget.transform.position, attackrange2.transform.position, out hit)|Physics.Linecast(playertarget.transform.position, attackrange3.transform.position, out hit)){
						...
						if(hit.transform){
							GetComponent<AudioSource>().clip=hitt;
		...
		GetComponent<Rigidbody>().AddForce(Vector3.up * 1900);
						jumptime=1;
