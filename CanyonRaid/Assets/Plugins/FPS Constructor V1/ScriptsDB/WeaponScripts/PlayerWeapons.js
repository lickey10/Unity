...
function Update(){
	...
	if (Input.GetButton ("Fire1") && (autoFire || charge) && canFire){
...
function SelectWeapon(index : int){
	...
	for (var i : int=0 ;i<weapons.length; i++)	{
		if ((i != index && i != index+1) && weapons[i] != null){
...
function ActivateWeapon(){
	...
	if(weapons[selectedWeapon+1] != null){
	    weapons[selectedWeapon+1].BroadcastMessage("SelectWeapon");
	}
	if(weapons[selectedWeapon] != null){
...
static function ShowWeapon() {
	...
	if(PW.weapons[PW.selectedWeapon+1] != null)
	    PW.weapons[PW.selectedWeapon+1].gameObject.BroadcastMessage("SelectWeapon");
}
