...
///This is similar to mouseLook, but is only for visual sway on weapons
	...
	function Update () {
		...
		if(useWalkMotion){
		    ...
		    var relVelocity : Vector3;
		    if(PlayerWeapons.CM != null)
                relVelocity = transform.InverseTransformDirection(PlayerWeapons.CM.movement.velocity);			
			var zVal : float;
			var xVal : float;
			var xVal2 : float;
			...
			var s : float;
			if(PlayerWeapons.CM != null)
                s = Vector3(PlayerWeapons.CM.movement.velocity.x, 0, PlayerWeapons.CM.movement.velocity.z).magnitude/14;
