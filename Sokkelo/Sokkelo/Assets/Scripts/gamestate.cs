...
public class gamestate : MonoBehaviour {
	...
	private bool gameRunning = true;				//whether the game is actually being played or not
	...
	private bool gameRunning_default = true;
	...
	bool DisplayingMenu = false;//when true a menu is being displayed on the UI
	int coins = 0;
	int torchesLit = 0;
	float startTime = 0;
	int restSeconds;
...
//		if(gamestate.Instance.GetNumberOfLevels() == 1)
        SceneManager.LoadScene("Maze 1 - swipe");
//		else//load a menu
//			Application.LoadLevel("LevelMenu");
	...
	void Update() {
		...
		if (torchesLit >= 1) {//they won the level
			SetGamePaused(true);
			activeLevel++;
...
//		}
		...
		if (instance != null) {
			float guiTime = Time.time - instance.startTime;

			restSeconds = instance.countDownSeconds - ((int)guiTime);
			...
			//display the timer
			instance.roundedRestSeconds = Mathf.CeilToInt (restSeconds);
			instance.seconds = instance.roundedRestSeconds % 60;
			instance.minutes = instance.roundedRestSeconds / 60; 

			if (!gameRunning && !IsGameWon) {//they lost the game
				// display game over screen
//				GUI.color = Color.black;
//				GUIStyle labelStyle = new GUIStyle ();
//				labelStyle.fontSize = 55;
//
//				GUI.Label (new Rect ((Screen.width - 100) / 2, (Screen.height - 50) / 2, 100, 50), "Game Over!!!", labelStyle);

				//GameObject.Find ("Game Over").SetActive (true);

			} else if (IsGameWon) {//they won the game
				// display game over screen
				GUI.color = Color.black;
				GUIStyle labelStyle = new GUIStyle ();
				labelStyle.fontSize = 55;

				GUI.Label (new Rect ((Screen.width - 100) / 2, (Screen.height - 50) / 2, 100, 50), "You WON!!!", labelStyle);
			}
		}
	}
	...
	public void SetTimer(int AmountToAdjustInSeconds)
	{
		//timer += AmountToAdjustInSeconds;
		instance.countDownSeconds += AmountToAdjustInSeconds;
	}
...
//			levelProgress = activeLevel;
	...
	public void SetTorchesLit(int newTorchesLit)
	{
		torchesLit = newTorchesLit;
	}
